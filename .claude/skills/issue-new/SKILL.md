...
This skill drives the `issuectl intake` command group (issuectl ≥ 0.6.6). On
first use in a session, run `issuectl --version`; if `intake` is missing
(`issuectl intake --help` errors), the binary is too old — tell the user to
upgrade (`brew upgrade jarimustonen/issuectl/issuectl`, `cargo install issuectl
--force`, or the shell installer) and stop.
