...
(using the exact version pinned in `dist-workspace.toml`) via one GitHub Actions
workflow:

- `.github/workflows/release.yml` — generated by `dist`. On the `vX.Y.Z`
  tag push (the tag Shipshape creates): builds binaries for macOS (aarch64 +
  x86_64) and Linux musl (aarch64 + x86_64), creates a GitHub Release with tarballs +
  a shell installer, pushes the Homebrew formula to
...
When upgrading cargo-dist, change `cargo-dist-version` in
`dist-workspace.toml`, obtain that exact version in a disposable location, and
regenerate the workflow from the repository config. Do not use `dist init` or
hand-edit the generated workflow.

```sh
/path/to/pinned/dist generate
/path/to/pinned/dist generate --check
/path/to/pinned/dist plan --allow-dirty --output-format=json > /tmp/dist-plan.json
git diff -- dist-workspace.toml .github/workflows/release.yml
```

Review the generated diff and plan for the configured target matrix, custom
runner, shell and Homebrew installers, tap publish job, attestations, and tag
