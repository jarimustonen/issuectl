...
and handles JSON output for you.

- List open issues: `issuectl ls`
- Filter: `issuectl ls -t bug -p high -a alice`
...
- Show details for one: `issuectl show NN`
- Keyword search across title/slug/body: `issuectl search KEYWORD [--all]`
- JSON output (parseable): add `--json` to any of the above
- Stats: `issuectl stats [--json]`
...
Display results to the user from the CLI's table output. For programmatic
filtering, use `--json` and process with jq.
...
- `issuectl close NN` — defaults to `fixed` for bugs, `done` otherwise
- `issuectl close NN --status wontfix` — explicit closing status
- `issuectl close NN --commit HASH:summary` — also record a commit (repeatable)
...
2. Run `issuectl close NN [--status X] [--commit HASH:summary]`
...
close` for each. Confirm each one.

### Action: Update

Use `issuectl update NN` with one or more flags. The CLI updates frontmatter
and bumps `updated:` automatically. If the new status is a closing status,
the issue is also moved to `closed/` (same as `close`).
...
- `issuectl update 12 --status in-progress`
- `issuectl update 12 --assignee alice --status testing` (handoff to tester)
- `issuectl update 12 --add-commit "abc123:fix login state"`
- `issuectl update 12 --add-label backend --add-label api`
...
- **Epic** — does this belong to an existing epic? Check with `issuectl ls -t epic`
...
The CLI:
- Picks the next available number automatically
...
sections by editing the item.md directly. For epics, add `## Goal`,
`## Issues`, `## Phases`, and `## Notes` sections — the CLI does not write
these.
...
- **Closing statuses** also move the directory to `closed/`. Use `issuectl
  close` (or `update --status`) — never `git mv` by hand
